...
impl FromUnpackedRecordImplsGenerator {
    fn generate_from_unpacked_record_impl(
        ...
        let from_impl = scope
            ...
            .generic(CAP_GENERIC)
            .target_generic(CAP)
            .impl_trait(format!("From<{}>", unpacked));

        let from_fn = from_impl.new_fn("from").arg("from", unpacked).ret("Self");
        from_fn.line(format!(
            "Self::{}(from)",
...
impl FragmentGenerator for FromUnpackedRecordImplsGenerator {
    fn generate(&self, specs: &FragmentGeneratorSpecs, scope: &mut Scope) {
        ...
        Self::generate_from_unpacked_record_impl(record_spec, false, scope);

        Self::generate_from_unpacked_record_impl(record_spec, true, scope);
    }
}
