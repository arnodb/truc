...
fn main() {
    ...
    let value_0_id = definition
        .add_datum_allow_uninit::<usize, _>("value_0")
        .unwrap();
    definition.close_record_variant();

    definition.remove_datum(value_0_id).unwrap();
    definition
        .add_datum_allow_uninit::<usize, _>("value_1")
        .unwrap();
    definition.close_record_variant();

    let definition = definition.build();

    let mut file = File::create(out_dir_path.join("truc_asm_1_truc.rs")).unwrap();
    write!(
        file,
        "{}",
